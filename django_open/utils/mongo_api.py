...
if os.path.exists("{}/{}/{}".format(BASE_DIR, "ipam_boost", "conf.py")):
    from ipam_boost.conf import mongo_db_conf
...
class IpamOps(object):
    ...
    def post_success_ip(ip):
        ...
        my_mongo = MongoOps(db='IPAMData', coll='django_open_ipam_success_ip')
    ...
    def post_fail_ip(ip):
        ...
        """
        log_time = datetime.now().strftime("%Y-%m-%d")
        my_mongo = MongoOps(db='IPAMData', coll='django_open_ipam_fail_ip')
    ...
    def post_update_ip(ip):
        ...
        """
        log_time = datetime.now().strftime("%Y-%m-%d")
        my_mongo = MongoOps(db='IPAMData', coll='django_open_ipam_update_ip')
    ...
    # IPAM批量读取coll内容，比如django_open_ipam_success_ip、django_open_ipam_fail_ip
    @staticmethod
    def get_bulk(coll):
